...
- 侧边栏和独立窗口共享同一个分析会话；
- 在任一界面点击“停止”都会停止所有捕获；
- 只关闭一个界面时，只要另一个界面仍打开，捕获会继续；
- 导出按钮仅在暂停视频并收到详细暂停画面后可用；
- 轨迹强度只改变渲染不透明度，不改变测量到的电平。

### 测量结果与 DaVinci Resolve 不同

版本 1.1 分析的是 Chrome 呈现的可见 SDR 输出。它无法访问原始视频文件、显示呈现之前的信号或完整的色彩元数据。这些示波器仅用于观察，并非经过校准的广播级测量工具。
...
- 实时分析、独立分析窗口、轨迹强度、RGB 最小/最大值，以及可导出的详细暂停画面。

版本 1.1 不支持 Shorts、YouTube Music、嵌入式播放器、全屏、YouTube 迷你播放器、画中画模式或经过校准的 HDR 分析。
