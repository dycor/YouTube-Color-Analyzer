...
该扩展只观察画面，不会修改视频、视频源文件或其渲染结果。它不包含任何后端服务，也不会向互联网发送任何数据。扩展不会自动保存图像；当视频暂停并生成详细帧时，用户可以明确选择将实际分析的图像导出为本地 PNG。
...
- 共享同一捕获会话的侧边栏和可分离分析窗口；
- 最高 512 个水平列的实时分析，以及最高 1920 个水平列的暂停详细分析；
- 轨迹强度调节、RGB 最小值/最大值，以及对暂停详细帧的明确本地 PNG 导出；
...
8. 可选择“在窗口中打开”来使用可分离视图。暂停视频即可启用详细测量和明确的 PNG 导出操作。
...
RGBA 图像始终保留在离屏文档和计算 worker 中。侧边栏和可分离窗口仅接收绘制示波器所需的紧凑型强度图。只有当用户明确请求本地 PNG 导出时，实际的详细帧才会提供给界面；扩展绝不会自动保存或上传该图像。
